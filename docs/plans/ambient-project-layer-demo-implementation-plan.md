...
> UI 范围更新（2026-08-13）：本计划中早期“三栏伴随面板”和“完整编辑”描述已由 [Inline 项目卡片产品边界](../architecture/inline-panel-product-boundary.md) 取代。后续实现以 Inline 状态操作 + Plane CTA 为准，不建设 Ambient Fullscreen 或独立完整看板。
...
Codex 正常工作
  ...
  -> 后台同步到 Plane
  -> 用户在 Inline 卡片中查看相关工作、修改状态，或进入 Plane 完整管理
```
...
- 当前项目摘要、同步健康状态和约 3–5 个相关工作项查询。
- 修改工作项状态，供 Inline 拖拽和状态菜单共用。
- 返回 Plane 项目或工作项 URL，供“在 Plane 中打开 ↗”使用。

所有修改经服务端写 Plane；浏览器代码不接触 Plane API Key。
现有完整编辑、合并、归档、删除、筛选和手动重试接口属于遗留实现，不进入 Inline 产品路径，也不在本阶段继续扩展。

#### D3-2 面板 UI

将现有三栏原型收敛为 Inline card，不做完整 Kanban：

- 顶部：当前项目和同步健康状态。
- 查看切换：“全部”以及 `Captured`、`Planned`、`In progress`、`Done`；状态项同时作为拖拽落点。
- 主体：约 3–5 个相关工作项，采用 Plane 卡片层级。
- 交互：拖拽或状态菜单修改 `Captured`、`Planned`、`In progress`、`Done`。
- 反馈：保存中、已同步、失败回滚。
- 底部：唯一主要 CTA “在 Plane 中打开 ↗”。

除内置的“全部 + 四状态”查看切换外，复杂筛选、完整编辑、创建、合并、归档、删除、分析和批量操作不进入 Inline，继续由 Plane 提供。自动链路仍不拥有删除权限。
...
- 验证目标 Codex 构建是否能显示并操作 MCP Inline UI。
- 记录 Inline 是否稳定渲染、重新打开和传递组件私有 metadata；Fullscreen 不是产品要求。
- 宿主不渲染时使用 4318 的同一轻量 UI 做开发/降级验收。
- 不使用 CDP，也不尝试永久侧边栏注入。
...
4. MCP App Inline 项目卡片，以及仅用于开发和宿主降级的 4318 同款页面。
