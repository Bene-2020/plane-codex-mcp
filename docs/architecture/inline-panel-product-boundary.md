...
任何无法进入正常已绑定项目卡片的失败都复用同一张 Plane 风格 Inline 异常卡片：宽度 `100%`、最大宽度 `720px`，仅显示标题“项目面板暂时不可用”和说明“暂时无法加载项目面板，请稍后再试。”。工具错误、缺少或非法 bootstrap、宿主连接失败、bridge/HTTP/网络失败、服务重启后的会话失效和首次摘要加载失败都归入该状态；不显示具体原因、绑定引导、步骤、Retry 或 Plane CTA。正常 bootstrap 收到前的首次等待仍使用上面的 Loading 骨架。

## Inline card 必须承担

- 展示当前 Plane 项目名称和同步健康状态。
- 展示约 3–5 个与当前工作最相关的工作项，而不是完整项目列表。
- “全部”视图不按更新时间直接截取全局最近 5 条，而是优先呈现用户当前最需要关注的未完成事项：从 `In Progress`、`Todo`、`Backlog` 各取最近更新的 1 条，再按 `In Progress` > `Todo` > `Backlog` 的优先级、同状态内按更新时间倒序补足。项目至少有 4 条未完成项时展示 4 条未完成项，并在存在已完成项时保留 1 条最近更新的 `Done` 作为近期成果反馈；未完成项不足 4 条时先展示全部未完成项，再用最近更新的 `Done` 补足，总数仍不超过 5 条。
- 单状态视图必须从该状态的完整非归档工作项集合中独立按更新时间倒序选取最多 5 条。例如 Backlog 有 6 条时显示最近更新的 5 条，有 3 条时显示全部 3 条；不得先截取“全部”视图的 5 条再做状态过滤。
...
- “全部”视图在对应状态非空时覆盖 `In Progress`、`Todo` 和 `Backlog`；至少有 4 条未完成项时采用“4 条未完成 + 最多 1 条最新 Done”，未完成项不足 4 条时才用 Done 补位。单状态视图独立展示该状态最近更新的最多 5 条，不受“全部”视图候选集合限制。
- 成功、失败和回滚均有直接可见反馈。
