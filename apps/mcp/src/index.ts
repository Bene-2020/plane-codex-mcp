...
const plane = createPlaneAdapter();
...
server.registerTool("list_projects", { description: "List Plane projects available for an explicit project-context choice.", inputSchema: z.object({}) }, async () => text(await plane.listProjects()));

server.registerTool("get_binding", { description: "Get the project context bound to a normalized cwd.", inputSchema: z.object({ cwd: z.string().min(1) }) }, async ({ cwd }) => text(storage.getContextByCwd(cwd)));

server.registerTool("bind_project", { description: "Bind a cwd to a Plane project after the user explicitly selected it.", inputSchema: bindingSchema }, async (input) => {
  const context = storage.bindContext(bindInput(input), false);
  return text(context);
});

server.registerTool("change_binding", { description: "Change the cwd's Plane project only after the user explicitly asks.", inputSchema: bindingSchema }, async (input) => text(storage.bindContext(bindInput(input), true)));

server.registerTool("record_project_events", { description: "Reliably accept one non-empty batch of meaningful project events for the current work turn. Plane synchronization is asynchronous.", inputSchema: eventBatchSchema }, async (input) => {
