...
const api = async <T,>(path: string, init?: RequestInit): Promise<T> => { const response = await fetch(path, { headers: { "Content-Type": "application/json" }, ...init }); if (!response.ok) throw new Error((await response.json()).error ?? `HTTP ${response.status}`); return await response.json() as T; };
...
function App() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const context = await api<Context | null>(`/api/context?cwd=${encodeURIComponent(cwd || window.localStorage.getItem("ambient.cwd") || ".")}`);
      if (!context) { setSummary(null); return; }
      const next = await api<Summary>(`/api/projects/${context.id}/summary`);
      setSummary(next); setCwd(next.context.canonicalCwd); window.localStorage.setItem("ambient.cwd", next.context.canonicalCwd); if (!selectedId && next.items[0]) setSelectedId(next.items[0].id);
  ...
  const updateContext = async (enabled: boolean) => { await api(`/api/projects/${summary.context.id}/auto-capture`, { method: "PATCH", body: JSON.stringify({ enabled }) }); setSummary({ ...summary, context: { ...summary.context, autoCaptureEnabled: enabled } }); };
  const edit = async (patch: Partial<Item>) => { if (!selected) return; const updated = await api<Item>(`/api/items/${selected.id}`, { method: "PATCH", body: JSON.stringify(patch) }); setSummary({ ...summary, items: summary.items.map((item) => item.id === updated.id ? { ...item, ...updated } : item) }); setMessage("已保存"); };
  const archive = async () => { if (!selected || !window.confirm("归档这条系统生成的记录？")) return; await api(`/api/items/${selected.id}/archive`, { method: "POST" }); await load(); setMessage("已归档"); };
  const remove = async () => { if (!selected || !window.confirm("删除这条系统生成的记录？此操作会同步到 Plane。")) return; await api(`/api/items/${selected.id}`, { method: "DELETE" }); await load(); setMessage("已删除"); };
  const merge = async (targetId: string) => { if (!selected || !window.confirm("把当前记录合并到选中的目标记录？")) return; await api(`/api/items/${selected.id}/merge/${targetId}`, { method: "POST" }); await load(); setMessage("已合并"); };
...
function FailureList({ failures, contextId, onDone }: { failures: Summary["failures"]; contextId: string; onDone: () => void }) { return <div className="failures">{failures.length ? failures.map((failure) => <div className="failure" key={failure.batch_id}><strong>{failure.batch_id}</strong><p>{failure.last_error ?? failure.status}</p><button className="ghost" onClick={async () => { await api(`/api/projects/${contextId}/retry/${failure.batch_id}`, { method: "POST" }); onDone(); }}>重试</button></div>) : <div className="blank">没有同步失败。</div>}</div>; }
