...
describe("record-or-ack integration", () => {
  it("allows a reviewed no-event turn to stop without creating a hook prompt", async () => {
    ...
    const client = new Client({ name: "ambient-record-or-ack-test", version: "0.1.0" });
    ...
    try {
      ...
      expect(await handleHook(JSON.stringify(stopInput), storage)).toMatchObject({ decision: "block" });
  ...
  it("keeps duplicate ack and record-or-ack ordering visible across MCP and Stop", async () => {
    ...
    const context = storage.bindContext({ cwd: "/work", planeBaseUrl: "https://plane.test", workspaceSlug: "ws", planeProjectId: "p" });
    const { server } = createMcpServer({ storage, plane: new FakePlaneAdapter() });
    const client = new Client({ name: "ambient-record-or-ack-order-test", version: "0.1.0" });
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
