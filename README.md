...
这是一个按 `docs/specs/ambient-project-layer-demo.md` 和 `docs/plans/ambient-project-layer-demo-implementation-plan.md` 实现的可运行 Demo：Codex 通过 MCP 将当前工作回合的项目事件可靠写入本地 SQLite Outbox，服务进程异步投射到 Plane，并提供自制的轻量 React Inline 项目卡片查看相关工作和直接修改状态。Plane 是用户可见项目数据的最终真相源和完整项目管理界面；Panel 不直接持有或调用 Plane API Key。
...
当前 UI 产品边界见 [Inline 项目卡片产品边界](docs/architecture/inline-panel-product-boundary.md)，最终视觉与交互基线为用户批准的 [Ambient Project Inline A 版](https://p.superdesign.dev/draft/2f45e055-ac0d-4035-b761-ce539f0229de)，加载状态基线为 [Plane 风格 5 条同构骨架](https://p.superdesign.dev/draft/bcc6772c-84fb-4894-92c9-f15279fac4e1)。Inline card 只展示约 3–5 个相关工作项，提供全宽“全部”与四状态查看切换，并支持把卡片拖到四状态更新状态；列表摘要明确区分当前显示数与 Plane 项目级总数。底部主要 CTA 为“在 Plane 中打开 ↗”。不建设 Ambient Fullscreen、独立 Web 完整看板或第二套项目管理界面。当前三栏详情面板是待收敛的遗留实现，不应继续扩展。
...
若本机仍有上线前使用 `event_*` 作为 Plane 来源标记的开发数据，请按 [数据库指南的来源 ID 迁移](docs/database-guide.md#上线前来源-id-迁移) 先 dry-run，再在停止 Codex Desktop 后执行一次性迁移；运行时不保留旧标记兼容逻辑。

## Codex 插件

`plugin/` 包含 `.codex-plugin/plugin.json`、`.mcp.json`、五种 Hook 配置和 `ambient-project` Skill。Codex 插件的 MCP/Hook 配置是单一命令字符串，没有按宿主 OS/架构条件选择命令的字段，因此正式发布采用平台专属包：每个包只携带一个固定 Node 22.22.1 sidecar，并由安装/marketplace 选择与宿主匹配的包。支持矩阵严格为：
...
仓库内的 `scripts/plugin-creator/validate_plugin.py` 与固定的 `requirements.txt` 提供可复现的 plugin-creator manifest contract 校验入口，CI 不依赖某台开发机的 `/Users/...` skill 路径。`.github/workflows/plugin-platform-native.yml` 在五个目标 runner 上分别打包并真实执行 bundled Node `--version`、MCP STDIO handshake 和五个 Hook；每个 job 使用 `fail-fast: false`，发布平台包 artifact 以及不含凭据、数据库、session token 或完整 transcript 的 `evidence.json`、`summary.md` 和 `checks.log`。

安装或升级插件后，必须在 Codex 的 Hook 管理界面人工复核并信任当前版本的五个 Hook，然后再新建 task 验收。`enabled = true` 只表示 Hook 已配置，不代表它已获准执行；如果运行时信任状态为 `modified`，Codex 会在启动 Hook 进程前拦截它。即使 `hooks.json` 文本没有变化，版本化缓存路径或解析后的命令变化也可能改变定义哈希，因此不要沿用上一版本的信任结果。典型症状是 MCP 工具仍可用，但新 task 没有项目上下文注入，正式插件数据库也没有该 session 的 `SessionStart` 或 `UserPromptSubmit` 审计。重新信任后，当前 task 的下一次提示可恢复 `UserPromptSubmit`；已经错过的 `SessionStart` 不会补跑，必须再新建 task 验证完整启动链。完整步骤见 [Codex Desktop 安装与真人验收](docs/codex-desktop-installation.md)。

MCP 暴露六个项目工作流工具和一个 UI bootstrap 工具：`list_projects`、`get_binding`、`open_project_panel`、`bind_project`、`change_binding`、`record_project_events`、`acknowledge_no_project_events`。Panel 另有仅组件可见的 `ambient_project_panel_request` server-tool bridge；它只代理固定的 Panel API 路径，不进入模型工具目录，且由 MCP 进程在同一动态 BFF 会话中带 token 请求。后者只持久化当前回合“已审查且无项目事件”的幂等确认，不创建 Plane 项目记录或 Outbox 批次。没有自动删除、跨项目移动或分配人员的模型工具。Panel 资源 URI 是 `ui://ambient-project/panel/v1.html`，使用官方 `_meta.ui.resourceUri` 连接工具和组件资源；它是自制 Codex Inline/宿主 UI，Plane SDK/API 不进入浏览器代码。
...
仓库开发数据库与 Codex Desktop 已安装插件数据库的路径选择、当前盘点和安全操作规则见 [数据库关系与操作指南](docs/database-guide.md)。两份数据库使用同一套 schema，但彼此独立且不会自动同步。
