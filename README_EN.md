...
Download an asset named like `ambient-project-layer-v0.1.0-<target>.zip` and extract it to a stable directory. The instructions below refer to its absolute path as `<RELEASE_DIR>`. This directory should directly contain `.agents/plugins/marketplace.json`.
