...
async function validatePackage(pluginRoot, { executeNative = true } = {}) {
  ...
  if (hooksConfig.hooks?.PostToolUse?.[0]?.matcher !== "^mcp__ambient_project__(record_project_events|acknowledge_no_project_events)$") throw new Error(`${pluginRoot}: PostToolUse must cover both record-or-ack tools`);
